...
// ---------------------------------------------------------------------------
// SignatureFields (reth-firehose)
// ---------------------------------------------------------------------------

impl reth_firehose::mapper::SignatureFields for BaseTxEnvelope {
    fn signature_fields(&self) -> (B256, B256, alloy_primitives::Bytes) {
        use reth_firehose::mapper::u64_to_trimmed_bytes;

        match self {
            // Legacy: V = chain_id * 2 + 35 + y_parity (EIP-155)
            //          or  27 + y_parity (pre-EIP-155)
            Self::Legacy(signed) => {
            ...
            // EIP-2930 / EIP-1559 / EIP-7702: V = 0 or 1 (y_parity).
            Self::Eip2930(signed) => typed_sig_fields(signed.signature()),
            Self::Eip1559(signed) => typed_sig_fields(signed.signature()),
            Self::Eip7702(signed) => typed_sig_fields(signed.signature()),
            // Deposit transactions carry no ECDSA signature — emit zero bytes.
            Self::Deposit(_) => (B256::ZERO, B256::ZERO, alloy_primitives::Bytes::new()),
        }
    }
}

fn typed_sig_fields(sig: &Signature) -> (B256, B256, alloy_primitives::Bytes) {
    use reth_firehose::mapper::u64_to_trimmed_bytes;
    let v = sig.v() as u64;
    (
        B256::new(sig.r().to_be_bytes::<32>()),
